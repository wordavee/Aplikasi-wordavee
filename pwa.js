(() => {
  ...
  if ("serviceWorker" in navigator && canRegister) {
    window.addEventListener("load", () => {
      navigator.serviceWorker.register("./sw.js").catch((error) => console.warn("Mode offline belum aktif", error));
    });
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    installBanner.querySelector(".mobile-install-action")?.addEventListener("click", installApp);
    installBanner.querySelector(".mobile-install-close")?.addEventListener("click", () => {
      try { sessionStorage.setItem("captionStudio.installDismissed", "1"); } catch (_) {}
      installBanner?.remove();
      installBanner = null;
    });
  });
  ...
  function wasDismissed() {
    try { return sessionStorage.getItem("captionStudio.installDismissed") === "1"; }
  ...
  function showInstallGuide() {
    ...
    const localNote = location.protocol === "file:"
    ...
    backdrop.querySelector(".pwa-guide-close")?.addEventListener("click", close);
